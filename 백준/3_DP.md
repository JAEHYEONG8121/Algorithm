...
- 이친수는 0과 1로만 이루어져있고, 0으로 시작하지 않고, 1이 두 번 연속으로 나타나지 않는 수이다.
- N(1 <= N <= 90)에서 N자리 이친수의 개수를 구하는 문제이다.
- n = 1부터 시작해서 이친수들을 찾아보았다.
...
n=4에서 n=2와 n=3의 경우가 모두 포함되어있다 : 10, 00, 01
n=5에서 n=3와 n=4의 경우가 모두 포함되어있다 : 100, 101, 000, 010, 001
-> 점화식 : dp[i] = dp[i-1] + d[i-2]
```

- 위를 통해 점화식을 찾았고, dp라는 리스트를 n+1 크기로 생성하고 dp[1] = 1로 초기화해둔 상태에서 for문을 2부터 돌면서 값을 저장한다.

**Review**
- 이제는 어렵지 않을 수도 있고 아닐 수도 있다!!
